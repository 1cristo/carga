...
import getAllCsvFiles from '@salesforce/apex/FileContentFetcher.getAllCsvFiles';
import fetchFileContentById from '@salesforce/apex/FileContentFetcher.fetchFileContentById';
...
export default class ImportarYmapearRegistros extends LightningElement {
    ...
    wiredCsvFiles({ error, data }) {
        if (data) {
            this.csvFileOptions = data.map(file => ({
                label: file.Title, // Muestra el título del archivo al usuario
                value: file.Id // Utiliza el Id del archivo como valor interno
            }));
        } else if (error) {
    ...
    handleCsvFileChange(event) {
        this.selectedCsvFileId = event.detail.value;
        fetchFileContentById({ documentId: this.selectedCsvFileId })
        .then(result => {
            // Decodifica el contenido de base64
            let csvContent = atob(result);
            // Divide el contenido en líneas y luego extrae los encabezados
        ...
        .catch(error => {
            ...
            this.updateImportMessage('Error al obtener el contenido del archivo.', 'error');
        });
    }
